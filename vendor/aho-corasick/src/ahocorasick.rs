...
#[derive(Debug)]
pub struct StreamChunkIter<R, S: StateID> {
    /// The AC automaton.
    fsm: Imp<S>,
...
impl<R: io::Read, S: StateID> StreamChunkIter<R, S> {
    pub fn new(ac: AhoCorasick<S>, rdr: R) -> StreamChunkIter<R, S> {
        assert!(
            ac.supports_stream(),
        ...
        let has_empty_match_at_end  = ac.is_match("");
        StreamChunkIter {
            fsm: ac.imp,
            prestate,
            rdr,
    ...
    pub fn next<'r>(&'r mut self) -> Option<io::Result<StreamChunk<'r>>> {
        loop {
            ...
            match result {
                None => {
                    ...
                    if self.state_id == self.fsm.start_state() {
